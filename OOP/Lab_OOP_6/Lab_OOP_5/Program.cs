...
namespace Lab_OOP_5
{
    struct Information  //1)структура - совокупность логически связанных данных различного типа, объе под одним ид
    {
        ...
        public void Print() 
        {
            Console.WriteLine($"Название: {book}  Краткое название: {person}  Автор: {name}");
        }
    }
    enum Book           //1)перечисление - набор логически связанных констант(по умолчанию int)
    {
        PrintEdition, Workbook, Magazine = 5, Book = 4
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Information inf = new Information { book = "Harry Potter and phylosophic rock", person = "Harry Potter", name = "John Rouling" };
            inf.Print();
            Console.WriteLine(inf.name);

            Book pe = Book.PrintEdition;
            Console.WriteLine(pe);
            Console.WriteLine((int)pe);

            Book workb = Book.Workbook;
            Console.WriteLine(workb);
            Console.WriteLine((int)workb);
